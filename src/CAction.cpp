...
string CAction::ToString(void)
{
    ...
    if (isMacro()) ((CMacroAction*)this)->DetermineInequalityConstraint();
...
bool CMacroAction::SubMacro(CMacroAction* m)
{
    vector<string> *as=this->LinearizeActNames();
    vector<string> *mas=m->LinearizeActNames();
      
    vector<string>::iterator it = search(as->begin(),as->end(),mas->begin(),mas->end());
