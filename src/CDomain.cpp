...
void CDomain::GetFlippingData()
{
	...
	if (incompatible_preds[i*n+j].first==1 && (*ppreds)[i]->GetPars()->Count()==(*ppreds)[j]->GetPars()->Count()){
	  ...
	  CPredicate* tmp_p2 = (*ppreds)[j];
	  //cout << tmp_p1->ToString(false) << tmp_p2->ToString(false) << endl;//debug
	  if (tmp_p1->GetPars()->Count() == incompatible_preds[i*n+j].second->size() ||
          (tmp_p1->GetPars()->Count() == (incompatible_preds[i*n+j].second->size() + 1) && strcasecmp(tmp_p1->GetName().data(),tmp_p2->GetName().data())==0)
         ){//thesecond predicate must "extend" the first one or one argument stands for "capacity"
	      flipping_data *tmpdata = new flipping_data();
	      tmpdata->starting=new CActionList();
	      tmpdata->finishing=new CActionList();
	      tmpdata->p1=tmp_p1;
