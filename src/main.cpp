...
*/
	...
	if (data.optimization){
		...
		*/
		...
		if (data.macrosx==1 || data.macrosx==5){ //"csm" macros
		...
		if (data.macrosx==21 || data.macrosx==25){ //"csm" macros
		   //data.pdom->IdentifyIncomaptiblePredicates();
		   //data.pdom->OutIncompatiblePreds(cout);
		   //data.pdom->GetFlippingData();
		   //data.pdom->OutFlippingData(cout);
          ...
          learn.Initialize();
		  learn.LearnEntanglements();
          learn.LearnMacrosFromFlips(data.macrosx==25);
          learn.ApplyEntanglements(true);
		  DomainToPDDL();
		  ProblemsToPDDL();
		}
		if (data.macrosx==6){ //"AMG" macros
		   //data.pdom->IdentifyIncomaptiblePredicates();
		   //data.pdom->OutIncompatiblePreds(cout);
		   //data.pdom->GetFlippingData();
		   //data.pdom->OutFlippingData(cout);
		  data.init=data.goal=true;
		  learn.LearnMacrosFromAMG();
		  DomainToPDDL();
		  ProblemsToPDDL();
		}
		if (data.macrosx==101 || data.macrosx==105){ //"flipping" macros
		   //data.pdom->IdentifyIncomaptiblePredicates();
		   //data.pdom->OutIncompatiblePreds(cout);
		   //data.pdom->GetFlippingData();
		   //data.pdom->OutFlippingData(cout);
		  learn.Initialize();
          data.init=data.goal=true;
		  learn.LearnMacrosFromFlips(data.macrosx==105);
          int n=data.train->size();
          for (int i=0;i<n;i++){
            (*data.train)[i]->GetPlan()->ApplyMacros(data.pdom->GetActions());  
          }
          learn.LearnMacrosFromAMG();
          //while (learn.LearnMacrosFromStaticPreconditions()){
          /*
          for (int i=0;i<n;i++){
            (*data.train)[i]->GetPlan()->UnfoldMacros(data.pdom->GetActions());
            (*data.train)[i]->GetPlan()->ApplyMacros(data.pdom->GetActions());  
          }
          */
         // learn.countFrequency();
         // learn.EliminateInfrequentMacros(data.train->size()*2);
          DomainToPDDL();
		  ProblemsToPDDL();
         // PlansToPDDL();
		}
		if (data.macrosx==106 || data.macrosx==107){
          cout << "importing macros"<< endl;
		  data.pdom->ImportMacros(&mcr_stuff);
          //data.goal=true;
          learn.Initialize();  
          //learn.countFrequency();
          if (data.entanglements) learn.LearnEntanglements(); else learn.countFrequency();
          learn.EliminateInfrequentMacros(data.train->size());
          if (data.entanglements) learn.ApplyEntanglements(true);
          if (data.macrosx==107){
		...
		if (data.macrosx==511 || data.macrosx==515){ //"flipping" macros
		   //data.pdom->IdentifyIncomaptiblePredicates();
		   //data.pdom->OutIncompatiblePreds(cout);
		   //data.pdom->GetFlippingData();
		   //data.pdom->OutFlippingData(cout);
		  learn.Initialize();
		  data.goal=true;
		  learn.LearnMacrosFromFlips(data.macrosx==515);
		  //remove "flipping operators"
		  int n=data.pdom->GetActions()->Count()-1;
		  CActionList *acts=data.pdom->GetActions();
		  while (n>=0&&(*acts)[n]->isMacro()){
          ...
          n=data.pprob->size();
          for (int i=0;i<n;i++){
            (*data.pprob)[i]->GetPlan()->ApplyMacros(data.pdom->GetActions());  
          }
          PlansToPDDL();
		}
