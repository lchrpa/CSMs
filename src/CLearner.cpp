...
void CLearner::countFrequency(){
	...
	memset(frequency, 0, data.pdom->GetActions()->Count()*sizeof(int));
    for (int i=0;i<data.train->size();i++){
		CProblem *prob=(*data.train)[i];
		for (int j=0;j<prob->GetPlan()->Length();j++){
			CAction *act=(*prob->GetPlan())[j];
...
void CLearner::EliminateInfrequentMacros(int freq)
{
   ...
   for(int i=0;i<acts->Count();i++){
      //debug
      (*acts)[i]->GenerateStatGraph();
      (*acts)[i]->GenerateComponents();
      cout << (*acts)[i]->GetActName() << ": " << *(frequency+i+rem) << " is macro " << (*acts)[i]->isMacro() << " - AMG components:" << (*acts)[i]->ComponentCount() <<endl;
      if (!(*acts)[i]->isMacro()) continue;
...
int CLearner::LearnMacrosFromFlips(bool no_im_args)
{
  ...
  while( (rec=data.pdom->NextFlippingItem()) != NULL){
	  ...
	  for (int i=0;i<plan->Length();i++){
	     ...
	     if (rec->starting->FindProperAction((*plan)[i]->GetActName(),s)!=NULL){
            ...
            CPredicate *ref_pred2 = (*(*plan)[i]->GetNegEff())[(*plan)[i]->GetNegEff()->FindProperPredicate(rec->p1->GetName())];
		...
		while (!closed && j<plan->Length()){
		  if (
               rec->finishing->FindProperAction((*plan)[j]->GetActName(),s)!=NULL && (*plan)[j]->GetNegEff()->Find(ref_pred)!=-1 &&
               rec->finishing->FindProperAction((*plan)[j]->GetActName(),s)!=NULL && (*plan)[j]->GetPosEff()->Find(ref_pred2)!=-1
             ){ //the "closing action is found
		    closed=true;
		  }
		  if (closed || ((*plan)[j]->GetPrec()->Find(ref_pred)!=-1 && (
                (*plan)[j]->GetNegEff()->Find(ref_pred)==-1 || (*plan)[j]->GetPosEff()->Find(ref_pred)!=-1)) /*|| !ref_act->IndependentWith((*plan)[j])*/){
  ...
  for (int xz=0;xz<data.pdom->GetActions()->Count();xz++)
    if ((*data.pdom->GetActions())[xz]->isMacro() && ((float)*(frequency+xz)>max_freq)) max_freq=(float)*(frequency+xz);
  
cout << "# of suggested macros: "<< sugg_macros.size() <<endl;
    
  vector<pair<CAction*,int> >::iterator mit=sugg_macros.begin();
  while (mit!=sugg_macros.end()){
     if (mit->second <   ((float) data.train->size())/2 || ((CMacroAction*)mit->first)->IsUninformative()) {mit=sugg_macros.erase(mit);continue;}
     cout << mit->first->GetActName() << " - #pars - " << mit->first->GetParams()->Count() << " - freq - " << mit->second << endl; //debug
     for (vector<pair<CAction*,int> >::iterator mit2=sugg_macros.begin();mit2!=sugg_macros.end();mit2++){
	   ...
	   if (mit->first->GetPrec()->Count() <= mit2->first->GetPrec()->Count() && mit->second<=mit2->second){
	      //cout << "To be erased " << mit->first->GetActName() << " because " << mit2->first->GetActName() << " # " <<mit2->second << endl;
          sugg_macros.erase(mit);
          //cout << "Afterwards " << mit->first->GetActName() << endl; 
          mit=sugg_macros.begin();
          break;
	  }
     }
     if (mit->second>max_freq) max_freq=mit->second;
     mit++;
  }
...
//  }
  ...
  for (mit=sugg_macros.begin();mit!=sugg_macros.end();mit++){
    ...
    //dismissing not so common macros
    if (((float)mit->second)<(max_freq/3.0f)) continue;  
    //dismissing existing macros
    int s;
    if (data.pdom->GetActions()->FindProperAction(mit->first->GetActName(),s)!=NULL) continue;
...
bool CLearner::LearnMacrosFromStaticPreconditions()
{
  ...
  vector<pair<CAction*,int> > sugg_macros;
   ...
   //debug
   //cout << "Suggested Macros: " << sugg_macros.size() << endl;
  ...
  if (sugg_macros.empty()) return false;
  for (vector<pair<CAction*,int> >::iterator mit=sugg_macros.begin();mit!=sugg_macros.end();mit++){
    //((CMacroAction*) mit->first)->DetermineInequalityConstraint();
    cout << mit->second << "x  Macro: " << mit->first->GetActName() << mit->first->GetParams()->ToString(true) << endl;//debug reasons
   // cout <<mit->first->GetPrec()->ToString(false) << endl;//debug reasons
...
void CLearner::LearnMacrosFromAMG()
{
   vector<pair<CAction*,int> > sugg_macros;
   ...
   for (deque<CProblem*>::iterator it_prob=data.train->begin();it_prob!=data.train->end();it_prob++){
	  ...
	  for (int it=0;it<plan->Length();it++){
	     ...
	     CAction *macro = (*data.pdom->GetActions())[s];
         for (int it2=it+1;it2<plan->Length();it2++){
	        if (macro->HasMeaningfulGoalEnt(1)) break;
            if (considered.find(it2) != considered.end()) continue;
            if (!ref_act->AchieverForGnd((*plan)[it2])) continue;
	        vector<sh_arg_str> sh_args;
		
            //if (((*(*plan)[ref_it]->GetPosEff())-(*(*plan)[ref_it]->GetPrec())).SubSetOf((*plan)[it2]->GetPrec()) && 
		    //!(*plan)[ref_it]->GetPosEff()->Disjunct((*plan)[it2]->GetNegEff())
		   //)
            ref_act->DetectSharedArgs((*plan)[it2],sh_args);
            CMacroAction *tmpmacro=new CMacroAction(ref_act,(*plan)[it2],sh_args);
            data.pdom->GetActions()->FindProperAction((*plan)[it2]->GetActName(),s2);
            macrox=new CMacroAction(macro,(*data.pdom->GetActions())[s2],sh_args);
            macrox->GenerateStatGraph();
		    macrox->GenerateComponents();
            //macro->GenerateStatGraph();
		    //macro->GenerateComponents();
            //(*data.pdom->GetActions())[s2]->GenerateStatGraph();
		    //(*data.pdom->GetActions())[s2]->GenerateComponents();            
            //debug
            //cout << "sugg macro: " << macrox->GetActName() << " components: " << macrox->ComponentCount() << "base actions components " << macro->ComponentCount() << "," << (*data.pdom->GetActions())[s2]->ComponentCount() << endl;
            
            if (macrox->ComponentCount()<=macro->ComponentCount()||macrox->ComponentCount()<=(*data.pdom->GetActions())[s2]->ComponentCount()){
		        
                //debug
            //cout << "2-sugg macro: " << macrox->GetActName() << " components: " << macrox->ComponentCount() << "base actions components " << macro->ComponentCount() << "," << (*data.pdom->GetActions())[s2]->ComponentCount() << endl;
            
                CPredicateList tmp_preds = ((*(*plan)[it2]->GetPrec())-(*ref_act->GetPosEff()))-(*ref_act->GetPrec());
                bool found = false;
                for (int j=0;j<tmp_preds.Count();j++){
                    if (!tmp_preds[j]->IsStatic()) continue;
                    sh_args.clear();
                    tmp_preds[j]->GetPars()->DetectShared(ref_act->GetParams(),sh_args);
                    if (!sh_args.empty()) {
                        found=true;
                        ref_act=tmpmacro;
                        //data.pdom->GetActions()->FindProperAction((*plan)[it2]->GetActName(),s2);
                        macro=macrox; //new CMacroAction(macro,(*data.pdom->GetActions())[s],sh_args);
                        for (int k=in_ma.back()+1;k<it2;k++) out_ma.push_back(k);
                        in_ma.push_back(it2);
                        break;
                    }
                }
		   if (found){ 
		       //debug
               //cout << "to be macro " << macro->GetActName() << " in_ma, out_ma size " <<in_ma.size() <<","<<out_ma.size() << endl;
               
               //   for (int i=(ref_it)+1;i<it2;i++) out_ma.push_back(i);
		       bool independent=true;
		  while (independent && !out_ma.empty()) {//upwards
		      deque<int>::iterator it_out=in_ma.begin(); 
		      while (*it_out < out_ma.front() && independent){
		  ...
		  while (independent && !out_ma.empty()) {//downwards
			 ...
			 it_out++;
		      ...
		      if (out_ma.empty()){
		          ...
		          //cout << ref_act->GetActName() << " is absorbed by " << (*plan)[it2]->GetActName() << " -- "; //debug reasons
		          
			  //    considered.insert(it2);
   ...
   for (vector<pair<CAction*,int> >::iterator mit=sugg_macros.begin();mit!=sugg_macros.end();mit++){
    //not enough macros w.r.t number of training plans
     cout << "Considering " << mit->first->GetActName() << "no of params " << mit->first->GetParams()->Count() << " with quantity " << mit->second << endl;
    if (!mit->first->isMacro() || ((CMacroAction*)mit->first)->IsUninformative() || (!mit->first->HasMeaningfulGoalEnt(1)) /*&& ((CMacroAction*)mit->first)->HasIntermediateAdditionalArgs()>1)*/) {mit=sugg_macros.erase(mit);mit--;continue;} 
    for (vector<pair<CAction*,int> >::iterator mit2=sugg_macros.begin();mit2!=sugg_macros.end();mit2++){
	  if (mit->first->GetActName()==mit2->first->GetActName() && mit->first->GetParams()->Count() < mit2->first->GetParams()->Count()){
	     mit->second+=mit2->second;
    ...
    for (vector<pair<CAction*,int> >::iterator mit2=sugg_macros.begin();mit2!=mit;mit2++){
	 ...
	 // } else {
	  // if (mit2->second < 2 * mit->second) {mit2=sugg_macros.erase(mit2);mit2--;} else {mit=sugg_macros.erase(mit);mit--;break;}
       ...
       if (!mit2->first->HasConnectedEnts() && ((CMacroAction*)mit->first)->SubMacro(((CMacroAction*)mit2->first))){
         mit2=sugg_macros.erase(mit2);mit2--;mit=sugg_macros.begin();break; //erase submacro of a connected macro 
       }
    }
