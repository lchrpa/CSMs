...
//creation of parameters from list of variables (without types)
CParameter::CParameter(vector<string> *vars,bool constant){
	ParamRecord tmp;
	//int i,n;
...
void CParameter::ToArray(vector<string> &p, vector<short> *q)
{
    int i=0;
    for(vector<short>::iterator it=q->begin();it!=q->end();it++){
        p[*it]=(*params)[i].constant;
        i++;
    }
}
