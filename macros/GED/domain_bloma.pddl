...
(:action begin-cut_mcr_end-cut-1_mcr_end-cut-2_mcr_begin-transpose-splice
:parameters ( ?x - object ?y - object ?yx2 - object ?xx3 - object)
:precondition (and (not (= ?x ?y))(idle)(cw ?x ?y)(not (= ?y ?yx2))(cw ?y ?yx2)(not (= ?xx3 ?x))(cw ?xx3 ?x)(stag_cw ?x ?yx2))
:effect (and (last-cut-point ?x)(s-first ?y)(s-last ?y)(cw ?x ?yx2)(splicing)(splice-point-1 ?xx3)(splice-point-2 ?x)(not (idle))(not (cw ?x ?y))(not (cw ?y ?yx2))(not (cutting))(not (cut-point-1 ?x))(not (cut-point-2 ?yx2))(not (have-cut))(not (cw ?xx3 ?x))(increase (total-cost) 2))
)
)