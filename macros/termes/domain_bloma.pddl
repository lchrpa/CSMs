...
(:action move_mcr_place-block
:parameters ( ?from - position ?to - position ?h - numb ?hafterx3 - numb)
:precondition (and (at ?from)(NEIGHBOR ?from ?to)(height ?from ?h)(height ?to ?h)(NEIGHBOR ?to ?from)(SUCC ?hafterx3 ?h)(has-block)(not (IS-DEPOT ?from)))
:effect (and (at ?to)(height ?from ?hafterx3)(not (at ?from))(not (height ?from ?h))(not (has-block)))
)
(:action move-down_mcr_remove-block
:parameters ( ?from - position ?hfrom - numb ?to - position ?hto - numb)
:precondition (and (at ?from)(NEIGHBOR ?from ?to)(height ?from ?hfrom)(height ?to ?hto)(SUCC ?hfrom ?hto)(NEIGHBOR ?to ?from)(not (has-block)))
:effect (and (at ?to)(height ?from ?hto)(has-block)(not (at ?from))(not (height ?from ?hfrom)))
)
)