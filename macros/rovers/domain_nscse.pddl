...
(stai_at ?x - rover ?y - waypoint)
(stai_at_soil_sample ?z - waypoint)
(stai_at_rock_sample ?z - waypoint)
)
(:action navigate
...
(:action navigate_mcr_sample_soil_mcr_navigate
:parameters ( ?x - rover ?y - waypoint ?z - waypoint ?sx3 - store)
:precondition (and (can_traverse ?x ?y ?z)(available ?x)(at ?x ?y)(visible ?y ?z)(at_soil_sample ?z)(equipped_for_soil_analysis ?x)(store_of ?sx3 ?x)(empty ?sx3)(can_traverse ?x ?z ?y)(visible ?z ?y)(stai_at ?x ?y)(stai_at_soil_sample ?z))
:effect (and (full ?sx3)(have_soil_analysis ?x ?z)(at ?x ?y)(not (empty ?sx3))(not (at_soil_sample ?z))(not (at ?x ?z)))
)
(:action navigate_mcr_sample_rock_mcr_drop_mcr_navigate
:parameters ( ?x - rover ?y - waypoint ?z - waypoint ?sx3 - store)
:precondition (and (can_traverse ?x ?y ?z)(available ?x)(at ?x ?y)(visible ?y ?z)(at_rock_sample ?z)(equipped_for_rock_analysis ?x)(store_of ?sx3 ?x)(empty ?sx3)(can_traverse ?x ?z ?y)(visible ?z ?y)(stai_at ?x ?y)(stai_at_rock_sample ?z))
:effect (and (have_rock_analysis ?x ?z)(empty ?sx3)(at ?x ?y)(not (at_rock_sample ?z))(not (full ?sx3))(not (at ?x ?z)))
)
)