...
(stai_at_rock_sample ?p - waypoint)
)
(:action navigate
...
(:action sample_rock_mcr_drop
:parameters ( ?x - rover ?s - store ?p - waypoint)
:precondition (and (at ?x ?p)(at_rock_sample ?p)(equipped_for_rock_analysis ?x)(store_of ?s ?x)(empty ?s)(stai_at_rock_sample ?p))
:effect (and (have_rock_analysis ?x ?p)(empty ?s)(not (at_rock_sample ?p))(not (full ?s)))
)
)