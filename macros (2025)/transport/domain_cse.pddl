; Generated by PTT
(define (domain transport)
(:requirements :typing :equality)
(:types size location locatable - object
 vehicle package - locatable
)
(:predicates (road ?l1 - location ?l2 - location)
(at ?x - locatable ?v - location)
(in ?x - package ?v - vehicle)
(capacity ?v - vehicle ?s1 - size)
(capacity-predecessor ?s1 - size ?s2 - size)
(stai_at ?p - package ?l - location)
(stai_capacity ?v - vehicle ?s2 - size)
(stag_at ?p - package ?l2x5 - location)
)
(:action drive
:parameters ( ?v - vehicle ?l1 - location ?l2 - location)
:precondition (and (at ?v ?l1)(road ?l1 ?l2))
:effect (and (not (at ?v ?l1))(at ?v ?l2))
)
(:action pick-up
:parameters ( ?v - vehicle ?l - location ?p - package ?s1 - size ?s2 - size)
:precondition (and (at ?v ?l)(at ?p ?l)(capacity-predecessor ?s1 ?s2)(capacity ?v ?s2))
:effect (and (not (at ?p ?l))(in ?p ?v)(capacity ?v ?s1)(not (capacity ?v ?s2)))
)
(:action drop
:parameters ( ?v - vehicle ?l - location ?p - package ?s1 - size ?s2 - size)
:precondition (and (at ?v ?l)(in ?p ?v)(capacity-predecessor ?s1 ?s2)(capacity ?v ?s1))
:effect (and (not (in ?p ?v))(at ?p ?l)(capacity ?v ?s2)(not (capacity ?v ?s1)))
)
(:action pick-up_mcr_drive_mcr_drop
:parameters ( ?v - vehicle ?l - location ?p - package ?s1 - size ?s2 - size ?l2x5 - location)
:precondition (and (at ?v ?l)(at ?p ?l)(capacity-predecessor ?s1 ?s2)(capacity ?v ?s2)(road ?l ?l2x5)(stai_at ?p ?l)(stai_capacity ?v ?s2)(stag_at ?p ?l2x5)(not (= ?v ?p)))
:effect (and (at ?v ?l2x5)(at ?p ?l2x5)(capacity ?v ?s2)(not (at ?p ?l))(not (at ?v ?l))(not (in ?p ?v))(not (capacity ?v ?s1)))
)
)